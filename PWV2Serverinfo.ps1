...
function Get-ServerInformation {
    ...
    $netbios = $networkAdapter.DNSHostName
    $fqdn = $networkAdapter.DNSDomain
    $ipConfiguration = $networkAdapter.IPAddress
    $ipAddress = $ipConfiguration[0]
    $gateway = $networkAdapter.DefaultIPGateway
    ...
    $software = Get-WmiObject -Class Win32_Product | Select-Object -Property Name, Vendor, Version

    # Get installed Windows roles
    $windowsRoles = Get-WindowsFeature | Where-Object {$_.Installed -eq 'True'}
    ...
    $serverInfo = [PSCustomObject]@{
        ...
        'Installed Windows Roles' = $windowsRoles
    }
